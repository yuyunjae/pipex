...
/*   Updated: 2024/08/28 22:51:14 by yuyu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	pipex_setting(t_pipex *pipe_info, int argc, char **argv, char **env)
{
	...
	if (ft_strncmp(argv[0], "./", 2) == 0)
		argv[0] = &argv[0][2];
	pipe_info->pid = (pid_t *)ft_calloc(sizeof(pid_t)
			* (pipe_info->arg_count - 3), 1);
	if (!pipe_info->pid)
		error_occur("malloc_error");
	if (!ft_strncmp(pipe_info->command[1], "here_doc"
			, ft_strlen(pipe_info->command[1])))
		pipe_info->here_doc = 1;
}
...
static void	wait_process(t_pipex *pipe_info)
{
	...
	i = -1;
	exit_status = 0;
	if (pipe_info->here_doc == 1)
	{
		i++;
		free(pipe_info->here_doc_temp);
	}
